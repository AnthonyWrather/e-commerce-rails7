#!/bin/bash
# Script to create GitHub issues for Sprint 01
...
echo "Creating GitHub issues for Sprint 01..."
echo "Repository: $REPO"
echo ""

# Create milestone if it doesn't exist
echo "Creating milestone: $MILESTONE"
gh api repos/$REPO/milestones -f title="$MILESTONE" -f state="open" 2>/dev/null || echo "Milestone may already exist"

# Issue 1: Fix LetterOpenerWeb Production Exposure
echo "Creating Issue 1: Fix LetterOpenerWeb Production Exposure"
gh issue create \
  --repo "$REPO" \
  --title "[CRITICAL] Fix LetterOpenerWeb Production Exposure" \
  --label "security,critical,bug,sprint-01" \
  --body "## Problem
...
# Issue 2: Implement Rate Limiting
echo "Creating Issue 2: Implement Rate Limiting with Rack::Attack"
gh issue create \
  --repo "$REPO" \
  --title "[CRITICAL] Implement Rate Limiting with Rack::Attack" \
  --label "security,critical,enhancement,sprint-01" \
  --body "## Problem
...
echo "Creating Issue 3: Enable Content Security Policy"
gh issue create \
  --repo "$REPO" \
  --title "[HIGH] Enable Content Security Policy Headers" \
  --label "security,high-priority,enhancement,sprint-01" \
  --body "## Problem
...
echo "Creating Issue 4: Implement Functional Contact Form"
gh issue create \
  --repo "$REPO" \
  --title "[HIGH] Implement Functional Contact Form with Email Delivery" \
  --label "feature,high-priority,sprint-01" \
  --body "## Problem
...
echo "Creating Issue 5: Add Contact Form Tests"
gh issue create \
  --repo "$REPO" \
  --title "[HIGH] Add Contact Form Tests" \
  --label "testing,high-priority,sprint-01" \
  --body "## Problem
...
## Dependencies
Must be completed after implementing functional contact form

## Story Points
2"

# Issue 6: Add Comprehensive Webhook Tests
echo "Creating Issue 6: Add Comprehensive Webhook Tests"
gh issue create \
  --repo "$REPO" \
  --title "[HIGH] Add Comprehensive Webhook Controller Tests" \
  --label "testing,high-priority,critical-path,sprint-01" \
  --body "## Problem
...
# Issue 7: Create Reusable Stripe Webhook Test Helper
echo "Creating Issue 7: Create Reusable Stripe Webhook Test Helper"
gh issue create \
  --repo "$REPO" \
  --title "[MEDIUM] Create Reusable Stripe Webhook Test Helper" \
  --label "testing,medium-priority,developer-experience,sprint-01" \
  --body "## Problem
...
## Dependencies
Should be completed as part of webhook testing work

## Story Points
2"

# Issue 8: Extract Webhook Logic to OrderProcessor Service
echo "Creating Issue 8: Extract Webhook Logic to OrderProcessor Service"
gh issue create \
  --repo "$REPO" \
  --title "[HIGH] Refactor: Extract Webhook Logic to OrderProcessor Service" \
  --label "refactoring,high-priority,code-quality,sprint-01" \
  --body "## Problem
...
## Dependencies
Should be completed after webhook tests are in place

## Story Points
8"

# Issue 9: Implement Model Scopes
echo "Creating Issue 9: Implement Model Scopes"
gh issue create \
  --repo "$REPO" \
  --title "[MEDIUM] Add Model Scopes for Cleaner Queries" \
  --label "refactoring,medium-priority,code-quality,sprint-01" \
  --body "## Problem
...
echo "Creating Issue 10: Refactor Controllers to Use Scopes"
gh issue create \
  --repo "$REPO" \
...
## Dependencies
Must be completed after model scopes are implemented

## Story Points
2"

# Issue 11: Implement Comprehensive Error Logging
echo "Creating Issue 11: Implement Comprehensive Error Logging"
gh issue create \
  --repo "$REPO" \
  --title "[MEDIUM] Implement Comprehensive Error Logging & Monitoring" \
  --label "monitoring,medium-priority,developer-experience,sprint-01" \
  --body "## Problem
...
echo "4. Create a Sprint 01 project board"
echo ""
