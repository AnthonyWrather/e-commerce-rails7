...
class Admin::OrdersController < AdminController
  ...
  def index
    @not_fulfilled_pagy, @not_fulfilled_orders = pagy(Order.unfulfilled.order(created_at: :asc))
    @fulfilled_pagy, @fulfilled_orders = pagy(Order.fulfilled.order(created_at: :desc),
  ...
  def admin_order_params
    params.require(:order).permit(:customer_email, :fulfilled, :total, :address, :name, :phone, :billing_address,
                                  :billing_name, :payment_status, :payment_id, :shipping_cost, :shipping_description,
                                  :shipping_id)
  end
end
